...
def process_single_set(
    i, sample_set, feature_sets, weights, sources, n_background, nfeatures, n_features
):
    fset = feature_sets[i]
    ...
    # Now sum the corresponding weights for the overlapping genes
    overlap_weights = [weight_ids[fset_ids.index(gene)] for gene in overlap_genes] if overlap_genes else []
    overlap_size = sum(overlap_weights)  # Sum of weights in fset
    overlap_size = float(overlap_size)
    overlap_size = round(overlap_size, 2)  # Ensure overlap_size is a float if your index is float

    overlaps = np.zeros(n_features, dtype=bool)
    overlaps[[fset_ids.index(gene) for gene in overlap_genes]] = True  # Marking overlaps

    # Load probability data for the source
    source = sources[i]
    P = pd.read_csv(f"probabilities_harmonic/probabilities_source_{source}.csv", index_col=0)

    p_value = 0
    if overlap_size in P.index:
        score_row = P.loc[overlap_size, :]
        for index, value in score_row.items():
            prob = value * hypergeom.pmf(int(index), n_background, len(fset), nfeatures)
            p_value += prob

    return overlap_size, overlaps, p_value
...
def get_pvals_fuzzy(sample, feature_sets, starts, offsets, n_background, n_features, sources, weights):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(process_single_set)(
            i, sample_set, feature_sets, weights, sources, n_background, sample.size, n_features
        )
...
def process_source(source, group, factor=100):
    ...
    output_dir = "probabilities_harmonic"
...
def run_ora(mat, net, source='source', target='target', weight = 'weight', n_up=None, n_bottom=0, n_background=None, min_n=5, seed=42,
